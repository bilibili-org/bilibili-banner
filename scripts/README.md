...
- `v1`：通过 Puppeteer 模拟交互，反推出旧版多图层 Banner 的运动参数。
- `v2`：直接解析首页 HTML 中内嵌的 `split_layer` 配置，再按官方规则下载资源并落盘。

## 一、V1：鼠标模拟计算运动参数

`v1` 抓取逻辑会模拟鼠标移动，自动分析 B 站旧版动态 Banner 的图层变换规律。

> 此方式无法捕获 Canvas 图层的资源和参数。
...
`v1` 的自动捕获方案本质上是“根据页面行为反推出一套近似参数”。虽然当前项目的 `v1` 效果已经能做到比较接近官方，但仍存在两个重要问题：

1. `blur`、`opacity`、`scaleSpeed`、`rotateSpeed`等参数，是较后期才逐步补进项目的grab脚本中的
2. 很多早期抓取的配置数据，当时并没有捕获这些关键参数，因此即使现在完善了渲染逻辑，旧配置数据本身仍然缺少关键参数

这会导致**早期配置即便能完整渲染，但交互效果也始终和官方有较大差异**

为了解决这个问题，我尝试在 [Wayback Machine][Wayback-Machine] 里找完整记录了 banner 交互的页面，但大多数存档页面的 banner 部分都无法顺利渲染，因此转而开始逆向 B 站官网。

观察浏览器控制台请求记录，首页里并没有出现 BAC 仓库中记录的 `x/web-show/page/header` 这类接口请求，再结合页面和脚本一起分析，最终确认现在的 banner 配置其实是直接内嵌在 HTML 里下发的，而且相关渲染逻辑也比较清晰。

因此项目新增了 `v2` 引擎。这样可以将部分旧 banner 的配置更新到 v2 版本，效果会更接近官方；同时也保留 `v1` 的鼠标模拟方式，用来兼容旧配置。
...
鉴于 B 站 Banner 的时效性，若需回溯已下线的往期 Banner，目前可行的数据源仍主要依赖 [Wayback Machine][Wayback-Machine]

由于 Wayback Machine 的采集机制限制，快照质量参差不齐。以下仅收录经测试可还原完整 Banner 交互的数据源：
